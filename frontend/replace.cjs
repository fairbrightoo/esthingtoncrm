...
// From '{/* Section 1.5: Pending MD-Approval Payments */}'
// To just before '</main>'

const startStr = '{/* Section 1.5: Pending MD-Approval Payments */}';
const endStr = '</main>';
...
// Find the </>\n)}... string before </main>
// We'll just replace everything from startIdx up to endIdx
...
let finalContent = content.substring(0, startIdx) + newContentStr + '\n' + content.substring(endIdx);

// Also need to add Eye, FileText, AlertCircle to imports
if (!finalContent.includes('Eye,')) {
    finalContent = finalContent.replace("lucide-react';", "Eye, FileText, AlertCircle, lucide-react';");
}
// Also add getPaymentTypeLabel
if (!finalContent.includes('getPaymentTypeLabel')) {
    const importStr = "import { getPaymentTypeLabel } from './ManagingDirectorDashboard';\n";
    finalContent = importStr + finalContent;
}
