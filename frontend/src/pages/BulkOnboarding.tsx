...
import { Upload, AlertCircle, CheckCircle, FileUp, Download, Building } from 'lucide-react';
...
export const BulkOnboarding = () => {
    ...
    useEffect(() => {
        // Only load companies if the user is a Super Admin
        if (user?.role === 'SUPER_ADMIN') {
            fetchCompanies();
        } else {
            // For branch admins, target company is their own company automatically
    ...
    const handleUpload = async () => {
        if (!file || !token || !selectedCompany) return;
    ...
    const handleDownloadTemplate = () => {
        const headers = "full_name,phone_number,email,assigned_branch,role,monthly_salary,commission_rate,date_of_birth\nExample Name,+23412345678,example@email.com,Abuja Branch,MARKETER,100000,5.0,1990-05-24\n";
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-800">Global Bulk Onboarding</h1>
                <button 
                    onClick={handleDownloadTemplate}
                    className="flex items-center space-x-2 text-primary-600 bg-primary-50 px-4 py-2 rounded-lg font-bold hover:bg-primary-100 transition-colors border border-primary-200"
                >
                    <Download size={18} />
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 border border-gray-200">
                {user?.role === 'SUPER_ADMIN' && (
                    <div className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-100">
                        ...
                        >
                            <option value="" disabled>Select a Subsidiary to attach these employees to...</option>
                            {companies.map(c => (
                                <option key={c.id} value={c.id}>{c.name}</option>
                            ))}
                        </select>
                        <p className="text-xs text-gray-500 mt-2">All users in the CSV will be placed inside this company framework.</p>
                    </div>
                )}
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-10 flex flex-col items-center justify-center text-center bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer relative">
                    ...
                    <p className="text-sm text-gray-500 mt-2">Supports Variable Roles, Salaries, and Branches.</p>
                </div>

                <div className="mt-6 flex justify-end">
                    <button
                        onClick={handleUpload}
                        disabled={!file || uploading || !selectedCompany}
                        className={`flex items-center space-x-2 px-6 py-3 rounded-lg text-white font-medium shadow-md transition-all ${(!file || uploading || !selectedCompany) ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 hover:shadow-lg'
                            }`}
                    >
                        {uploading ? (
